...
PEER0_ORGXXX_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/organizations/peerOrganizations/orgXXX.example.com/peers/peer0.orgXXX.example.com/tls/ca.crt
...
setGlobals() {
  ...
  if [ $ORG -eq 1 ]; then
  ...
  elif [ $ORG -eq XXX ]; then
    CORE_PEER_LOCALMSPID="OrgXXXMSP"
    CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_ORGXXX_CA
    ...
    CORE_PEER_ADDRESS=peer0.orgXXX.example.com:11051
  else
