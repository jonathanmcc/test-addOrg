...
git checkout c1424748b0d27351613269876376ae253951010c

echo ""
echo "Install Hyperledger Fabric 2.0.0 binaries and coker images ..."
sg docker -c "curl -sSL https://bit.ly/2ysbOFE | bash -s -- 2.2.0 1.4.6 0.4.18 -s"
mkdir -p $GOPATH/bin/
...
cd $GOPATH/src/github.com/hyperledger/fabric-samples/
git clone https://github.com/jonathanmcc/test-addOrg.git

echo ""
echo "Now start test-network ..."
cd $GOPATH/src/github.com/hyperledger/fabric-samples/test-addOrg/scripts/org3-scripts
chmod +x envVarCLI.sh
chmod +x step1org3.sh
chmod +x step2org3.sh

cd $GOPATH/src/github.com/hyperledger/fabric-samples/test-addOrg/scripts/
chmod +x createChannel.sh
chmod +x deployCC.sh
chmod +x envVar.sh

cd $GOPATH/src/github.com/hyperledger/fabric-samples/test-addOrg/
chmod +x network.sh
...
sg docker -c "./network.sh deployCC"
